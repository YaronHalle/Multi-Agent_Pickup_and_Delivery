...
class DeliveryStation(object):
    ...
    def is_delivery_spot_free(self):
        ...
        return (self.delivery_pos not in self.solver.get_shelves_locations()) and self.current_processed_task is None
    ...
    def available_spots_count(self):
        ...
        if self.next_waiting_index is not None:
            for index in range(self.next_waiting_index, self.queue_size):
                if tuple(self.waiting_locations[index]) not in self.solver.get_shelves_locations():
                    count += 1
    ...
    def progress_queue(self, index):
        ...
        if index == -1:
            ...
            # Making sure that the first task in queue has a carrying agent and can be forwarded
            next_task_name = self.tasks_queue[0]
            if next_task_name not in self.solver.get_tasks_to_agents():
                # The first waiting task has no agent to forward it. Therefore, the entire queue is stuck -> stop.
                return
        ...
        for i in range(queue_progress_start_ind, len(self.tasks_queue)):
            ...
            # Asserting that the next task in queue has a carrying agent and can be forwarded
            if next_task_name not in self.solver.get_tasks_to_agents():
                # No agent carries this pod. Therefore, the rest of the queue is stuck -> break the for loop.
                break
